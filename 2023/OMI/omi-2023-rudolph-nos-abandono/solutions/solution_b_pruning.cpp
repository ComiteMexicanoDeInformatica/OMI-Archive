 ...
 public:
  ...
  vector<bool> decifraBit(int num_bit) && {
    ...
    while (union_find.size() > 0) {
      ...
      bool raices_podadas = false;
      for (int r : raices) {
        const int tam = union_find.obtenTamComponente(r);
        if (tam > restantes_0s) {
          asignaValor(true, /*indices=*/union_find.obtenComponente(r),
                      /*salida=*/ret);
          raices_podadas = true;
        } else if (tam > restantes_1s) {
          asignaValor(false, /*indices=*/union_find.obtenComponente(r),
                      /*salida=*/ret);
          raices_podadas = true;
        }
      }
