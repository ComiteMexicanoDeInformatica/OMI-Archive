...
// freopen("output.txt", "wt", stdout);
  ...
  while (true) {
    ...
    if (rid != -1) {
      forn(j, m) { c[j] -= a[rid][j] * mr[rid] * mc[j]; }
      mr[rid] *= -1;
      forn(j, m) { c[j] += a[rid][j] * mr[rid] * mc[j]; }
    ...
    if (cid != -1) {
      forn(i, n) { r[i] -= a[i][cid] * mc[cid] * mr[i]; }
      mc[cid] *= -1;
      forn(i, n) { r[i] += a[i][cid] * mc[cid] * mr[i]; }
      c[cid] *= -1;
      continue;
  ...
  vector<int> rr, cc;
  forn(i, n) if (mr[i] == -1) rr.pb(i);
  forn(j, m) if (mc[j] == -1) cc.pb(j);

  cout << sz(rr) << " ";
  forn(i, sz(rr)) cout << rr[i] + 1 << " ";
  cout << endl;
  cout << sz(cc) << " ";
  forn(i, sz(cc)) cout << cc[i] + 1 << " ";
  cout << endl;
