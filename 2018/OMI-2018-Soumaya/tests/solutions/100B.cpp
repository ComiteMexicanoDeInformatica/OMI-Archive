...
pair<pair<int, int>, pair<int, int> > fn(int f, int c) {
  ...
  while (!cola.empty()) {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      hijo.fi += dy[k];
      hijo.se += dx[k];
      if (hijo.fi >= 1 && hijo.fi <= n && hijo.se >= 1 && hijo.se <= m &&
          !use[hijo.fi][hijo.se] && mdo[hijo.fi][hijo.se]) {
...
bool choca(pair<pair<int, int>, pair<int, int> > a,
           set<pair<pair<int, int>, pair<int, int> > >::iterator b) {
  if (a.r + 1 < b->l || a.d + 1 < b->t || a.l - 1 > b->r || a.t - 1 > b->d)
    return false;
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++) {
      cin >> xyz;
      mdo[i][j] = (xyz == '1') ? true : false;
    }
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++)
      if (mdo[i][j] && !use[i][j]) fig.insert(fn(i, j));
    ...
    for (ini = fig.begin(); ini != fig.end(); ini++) {
      ...
      for (; it != fig.end(); it++) {
        if (choca(newVal, it)) {
          ...
          newVal = {{min(newVal.t, it->t), max(newVal.d, it->d)},
                    {min(newVal.l, it->l), max(newVal.r, it->r)}};
        }
      }
  ...
  for (it = fig.begin(); it != fig.end(); it++) {
    for (int f = it->t; f <= it->d; f++)
      for (int c = it->l; c <= it->r; c++) salida[f][c] = true;
  }
