...
int Preguntar(int i, int j) {
  ...
  for (int k = 0; k < tesoros.size(); ++k) {
    if (tesoros[k].first <= i && tesoros[k].second <= j) respuesta++;
  }
  return respuesta;
}
