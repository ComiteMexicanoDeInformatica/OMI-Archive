...
ll fn(ll fila, ll columna, ll tiempo) {
  ...
  ll x = 0, y;
  bool useY = false;
  if (tiempo >= mdo[1][fila][columna] &&
      (tiempo - mdo[1][fila][columna]) % mdo[2][fila][columna] == 0)
    x = mdo[0][fila][columna];
...
int main() {
  ...
  cin >> n >> m >> t;
  for (ll i = 0; i < n; i++)
    for (ll j = 0; j < m; j++) cin >> mdo[1][i][j];
  for (ll i = 0; i < n; i++)
    for (ll j = 0; j < m; j++) cin >> mdo[2][i][j];
  for (ll i = 0; i < n; i++)
    for (ll j = 0; j < m; j++) cin >> mdo[0][i][j];

  ans = fn(0, 0, 1);
  for (ll i = 0; i < n; i++)
    for (ll j = 0; j < m; j++) ans = max(ans, fn(i, j, 1));
