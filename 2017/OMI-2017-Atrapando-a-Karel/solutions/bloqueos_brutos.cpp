...
void atrapando(int n) {
    ...
    while (keep) {
        //norte
        if (!horizontales[fila][columna]) {
            muro(fila, columna, NORTH);
        } else if (!verticales[fila][columna]) {
            muro(fila, columna, EAST);
        } else if (!horizontales[fila + 1][columna]) {
            muro(fila, columna, SOUTH);
        } else if (!verticales[fila][columna - 1]) {
            muro(fila, columna, WEST);
        } else {
            keep = false;
        }
    }
