...
int f(int a, int b) {
  ...
  return 2 * higher[b] - (a + b) < (a + b) - 2 * lower[a] ? higher[b]
                                                          : lower[a];
}
...
int main() {
  ...
  for (int i = 1; i < n; i++) S += abs(arr[i] - arr[i + 1]);
  ...
  for (int i = 1; i <= n; i++) {
    scanf("%d", &sol_user[i]);
    if ((inicial[i] && inicial[i] != sol_user[i]) ||
        (!inicial[i] && !bucket[sol_user[i]])) {
      printf("0\n");
      return 0;
    }
  }
  s_user = 0;
  for (int i = 1; i < n; i++) s_user += abs(sol_user[i] - sol_user[i + 1]);
  if (S != s_user) {
    printf("0\n");
    return 0;
  }
