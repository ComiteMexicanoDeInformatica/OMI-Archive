...
int main() {
  ...
  // SI EL TAMANIO DEL MAPA DE RESPUESTA ES DISTINTO DEL ORIGINAL, CALIFICA CON
  // 0
  if (n != N || m != M) {
  ...
  for (int fil = 1; fil <= N; ++fil) {
    ...
    for (int col = 0; col < M; ++col) {
      if (s[col] == '#')
        mapaFin[fil][col + 1] = ocupado;
      else if (s[col] == '.')
        mapaFin[fil][col + 1] = libre;
      else if (s[col] == 'F')
        mapaFin[fil][col + 1] = fuente;
      else {
        // USA OTROS CARACTERES ADEMAS DE LOS PERMITIDOS, CALIFICA CON 0
        std::cerr << "La posicion (" << fil << ", " << col
                  << ") tiene un caracter invalido\n";
        printf("%.2f\n", 0.0);
        return 0;
      }
    }
  ...
  for (int fil = 1; fil <= N; ++fil) {
    for (int col = 1; col <= M; ++col) {
      if ((mapaIni[fil][col] == ocupado && mapaFin[fil][col] != ocupado) ||
        ...
        std::cerr << "Las posiciones libres y ocupadas no se respetaron en el "
                     "resultado\n";
        printf("%.2f\n", 0.0);
        return 0;
      }
  ...
  for (int fil = 1; fil <= N; ++fil) {
    for (int col = 1; col <= M; ++col) {
      if (mapaFin[fil][col] == fuente)
        ++F;
      else if (mapaFin[fil][col] == libre)
        ++L;
      else if (mapaFin[fil][col] == agua)
        ++A;
    }
  }
  tot = F + L + A;
