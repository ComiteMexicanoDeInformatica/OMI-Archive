...
ll jump(ll houses) {
  currentIndex += houses;
  return housesMap[currentIndex] = currentNumber =
             avanza(houses) * directionSign;
}

void findSign() {
  ll secondHouse = jump(1);
  if (std::min(origin, secondHouse) < target &&
      std::max(origin, secondHouse) > target) {
...
void power2JumpsSearch() {
  ...
  while (1) {
    ...
    ll newSign =
        ...
        std::max(currentNumber - lastNumber, -(currentNumber - lastNumber));

    if (newSign != searchDirection ||
        (searchDirection == 1 && currentNumber >= target) ||
        (searchDirection == -1 && currentNumber <= target)) {
