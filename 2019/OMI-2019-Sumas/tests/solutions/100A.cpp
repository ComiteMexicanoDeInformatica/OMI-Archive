...
struct SubMatriz {
  ...
  int c2;
  ll w;
  bool operator<(const SubMatriz& b) const { return w < b.w; }
};
...
SubMatriz CalculaMaxValorEnPosicion(int fila_abajo, int col_der) {
  ...
  for (int fila_arriba = fila_abajo, max_longitud_base = col_der + 1;
       fila_arriba >= 0; --fila_arriba) {
    if (fila_arriba != fila_abajo &&
        Color[fila_arriba][col_der] == Color[fila_arriba + 1][col_der]) {
      return ret;
    }
    auto num_columnas_optimo_anterior =
        NumColumnasOptimo[fila_arriba][fila_abajo];
    auto suma_con_columna_unica =
        ObtenSuma(fila_arriba, col_der, fila_abajo, col_der);
    max_longitud_base =
        min(max_longitud_base, MaxPatronHorizontal[fila_arriba][col_der]);
    ...
    if (col_der > 0 && max_longitud_base > num_columnas_optimo_anterior) {
      ...
      auto suma_con_varias_columnas =
          ObtenSuma(fila_arriba, col_izq, fila_abajo, col_der);
      if (mejor_suma < suma_con_varias_columnas) {
        mejor_suma = suma_con_varias_columnas;
        NumColumnasOptimo[fila_arriba][fila_abajo] =
            num_columnas_optimo_anterior + 1;
        mejor_col_izq = col_izq;
      }
    }
    if (mejor_suma > ret.w) {
      ret = SubMatriz{fila_arriba, mejor_col_izq, fila_abajo, col_der,
                      mejor_suma};
    }
  }
  return ret;
