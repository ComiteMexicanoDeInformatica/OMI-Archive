...
void consumeChar(char c) {
  ...
  if (x != c) fail("unexpected character!");
}

long long readInt(long long lo, long long hi) {
  long long x;
  cin >> x;
  if (x < lo or x > hi) fail("out of bounds");
  return x;
}
...
i64 solve(i64 tx1, i64 ty1, i64 tx2, i64 ty2) {
  ...
  if (cnt && cnt != (tx2 - tx1 + 1) * (ty2 - ty1 + 1)) fail("not a chessboard");
  return tmp;
}
...
int main() {
  ...
  datain >> N >> M;
  for (i64 i = 0; i < N; i++)
    for (i64 j = 0; j < M; j++) datain >> a[i][j];

  for (i64 i = 0; i < N; i++)
    for (i64 j = 0; j < M; j++) {
      datain >> t[i][j];
      if ((i + j) & 1) t[i][j] = !t[i][j];
    }
  ...
  if (ans != ans2) fail("incorrect answer");
