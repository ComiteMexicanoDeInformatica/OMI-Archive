...
bool cmp(nodo a, nodo b) { return a.t < b.t; }
...
void upd(i64 x, i64 ax1, i64 ay1, i64 ax2, i64 ay2, i64 qx1, i64 qy1, i64 qx2,
         i64 qy2, i64 v) {
  updLazy(x);
  if (ax1 > ax2 or ay1 > ay2) return;
  if (ax1 > qx2 or ax2 < qx1 or ay1 > qy2 or ay2 < qy1) return;
...
i64 suma(i64 ax1, i64 ay1, i64 ax2, i64 ay2) {
  return sum[ax2][ay2] - sum[ax2][ay1 - 1] - sum[ax1 - 1][ay2] +
         sum[ax1 - 1][ay1 - 1];
}
int main() {
  optimiza_io cin >> N >> M >> K >> LIM;
  for (i64 i = 1; i <= N; i++)
    for (i64 j = 1; j <= M; j++) {
      cin >> a[i][j];
  ...
  cin >> Q;
  for (i64 i = 1; i <= Q; i++) cin >> q[i].i >> q[i].j >> q[i].x >> q[i].t;
  ...
  sort(q + 1, q + Q + 1, cmp);
  for (i64 i = 1; i <= Q; i++) {
    upd(0, 1, 1, N, M, max(1LL, q[i].i - K + 1), max(1LL, q[i].j - K + 1),
        min(N, q[i].i), min(M, q[i].j), q[i].x);
