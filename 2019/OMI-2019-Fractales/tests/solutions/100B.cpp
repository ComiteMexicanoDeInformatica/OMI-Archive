...
lld cnt(int x, int y) {
  ...
  if (!memo.count({x, y})) {
    ...
    if (c == 2)
      ...
      ans = cuadro[lv - 1] + cnt(x - pot3[lv - 1], pot3[lv - 1]) +
            cnt(pot3[lv - 1], y - pot3[lv - 1]);
    else if (c == 6)
      ans = 2 * cuadro[lv - 1] + cnt(x - pot3[lv - 1], y - 2 * pot3[lv - 1]) +
            cnt(x - pot3[lv - 1], pot3[lv - 1]) +
            cnt(pot3[lv - 1], y - 2 * pot3[lv - 1]);
    else if (c == 9)
      ans = 3 * cuadro[lv - 1] + 2 * cnt(x - 2 * pot3[lv - 1], pot3[lv - 1]) +
            2 * cnt(pot3[lv - 1], y - 2 * pot3[lv - 1]);
