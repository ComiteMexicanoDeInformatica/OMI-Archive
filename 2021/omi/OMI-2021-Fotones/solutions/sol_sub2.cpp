...
lli n, e, F, C, f[MAX_N], c[MAX_N], dirlaser, fe, ce, visitado[4][102][102],
    memoria[4][102][102], resexp;
char espejo[MAX_N], disp[102][102], dire;

int simula(int direccion, int fila, int columna, int experimento) {
  if (disp[fila][columna] == 'X')
    return 1;  // DEVUELVE 1 PARA DIFERENCIAR EN LA MEMORIA LOS QUE YA PASASTE
  if (visitado[direccion][fila][columna] == experimento) return -1;
  if (memoria[direccion][fila][columna])
    return memoria[direccion][fila][columna];  // SI YA LO VISITASTE

  visitado[direccion][fila][columna] = experimento;
...
int main() {
  ...
  rep(experimento, 1, e) {
    ...
    resexp = simula(dirlaser, fe, ce, experimento);
    if (resexp == -1)
      std::cout << resexp << nl;
    else
      std::cout << resexp - 1 << nl;
  }
